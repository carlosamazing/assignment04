...
#This treemap visualizes the cumulative gun deaths by county for New York (NY), Pennsylvania (PA), and New Jersey (NJ) from 1999 to 2019. 
#It highlights the disparities in gun deaths across counties within these states, with counties having fewer than 100 cumulative deaths combined into the "Other" category. 
#This information is crucial for policymakers, as it identifies regions with higher gun violence rates, allowing for targeted intervention and resource allocation.
```
...
county_chart <- 
  ggplot(filtered_data, aes(x = reorder(County, -Deaths), y = Deaths, fill = State)) +
  geom_bar(stat = "identity") +
  labs(
    title = "Total Gun Deaths by County for Selected States",
    subtitle = "Comparison of Gun Deaths by County for 1999, 2009, and 2019",
    caption = "Data: Centers for Disease Control and Prevention"
  ) +
  scale_fill_manual(
    values = c("NY" = "skyblue3", "PA" = "#002147", "NJ" = "darkgoldenrod"),
    name = "State"
  ) +
...
state_chart <- 
  ggplot(filtered_data, aes(x = State, y = Deaths, fill = as.factor(Year))) +
